...
// Version 14.2 (Robust Message Handling)
...
wss.on('connection', (ws, req) => {
    ...
    // RESTRUCTURED: The entire message handler is restructured for clarity and robustness.
    ws.on('message', async (message) => {
        // First, handle binary data (audio)
        ...
        // If it's not a buffer, it must be a text command (JSON)
        try {
            const data = JSON.parse(message.toString());

            // Check if the session is initialized for any command other than CONFIG
                ...
                case 'END_OF_STREAM': {
                    ...
                    if (transcript && transcript.trim()) {
                        ...
                        if (connectionMode === 'voice') {
                            ...
                            // Fallback to text if mode changed during transcription
