...
wss.on('connection', (ws) => {
    console.log('[WS] New connection established. Waiting for ping...');
        ...
        try {
            ...
            switch (data.type) {
                ...
                case 'INIT_SESSION':
                    ...
                    if (ws.readyState === 1) ws.send(JSON.stringify({ type: 'AI_RESPONSE', text: welcomeMessage }));
                ...
                case 'END_OF_STREAM':
                    ...
                    if (transcript.trim() && ws.readyState === 1) ws.send(JSON.stringify({ type: 'USER_TRANSCRIPT', text: transcript }));
                    break;
            ...
            if (transcript && transcript.trim()) {
                ...
                if (connectionMode === 'voice') await speakText(reply, ws, currentLanguage);
            }
        } catch (error) {
